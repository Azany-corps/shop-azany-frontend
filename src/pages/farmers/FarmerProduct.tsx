...
import Comments from "../../components/CustomerProfile/Comments";
...
const FarmerProduct: React.FC<FarmerProductsProps> = ({ productDetails, otherProducts }) => {
  ...
  return (
    <>
      <div className="py-3">
        <div className="w-[90%] mx-auto">
          <div className="py-4 relative">
            <Grid container spacing={2} className="!relative" sx={{ position: "relative !important" }}>
              <Grid item xs={12} md={8}>
                <div className="bg-white p-4 shadow-lg rounded-md flex gap-2 flex-row">
                  <div className="flex flex-col gap-2 w-2/3">
                    <img src={images[currentImageIndex]} alt="rice" className="h-[200px] w-[250px] object-contain" />
                    <div className="flex flex-row gap-2 cursor-pointer">
                      {images?.map(
                        (image: string, i: number) =>
                          i < 4 && <img src={image} alt="product" className="w-14 h-14" key={i} onClick={() => setCurrentImageIndex(i)} />
                      )}
                    </div>
                    <p>Share</p>
                  ...
                  <div className="flex flex-col gap-6 w-full">
                    <div>
                      <p className="font-medium text-[32px]">{productDetails?.product_name}</p>
                      <p className="font-medium text-[16px]">{productDetails?.product_details?.[0]?.specifications}</p>
                    </div>
                    ...
                    <div className="flex justify-between">
                      <div className="flex flex-row gap-2">
                        <BasicRating rating={productDetails?.rating ? productDetails?.rating : 0} />
                        <p className="text-[#515151]">{productDetails?.rating ? productDetails?.rating : 0}</p>
                      </div>
                      <p className="text-[#E51B48] underline">Add to Watchlist</p>
                    </div>
                    <hr className="border border-gray-300" />
                    <div className="justify-between ">
                      <p className="items-medium text-[32px]">{`${productDetails?.currency}   ${productDetails?.price}`}</p>
                    </div>
                    <div className="bg-[#F4F4F4] rounded-md p-4 gap-2 flex flex-col">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div>
                          <button className="border border-black py-2 px-4 rounded-md">{productDetails.price}</button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-5 justify-between">
                      ...
                      <button className="bg-[#E51B48] py-2 px-4 w-[180px] text-white rounded-md hover:bg-red-600" onClick={openModal}>
                        Buy Now
                      </button>
                    </div>
                  </div>
              ...
              >
                <div className="bg-white p-4 shadow-lg rounded-md flex gap-2 flex-col">
                  <div>
                    <p>DELIVERY</p>
                    <p className="text-[#515151]">No Import Fees Deposit & $30.89 Shipping to Kenya </p>
      ...
      <div className="mx-4">
        <Category products={otherProducts} styles={"text-[#132A13] bg-[#ECF39E] font-medium text-lg"} title={"Similar Prices"} country={"country"} />
      </div>
      <div className="mx-4">
        <Category
          products={otherProducts}
