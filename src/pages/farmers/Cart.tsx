...
import { calculateTotalPrice, getCartProducts, reomveProductFromCartService, updateProductQuantityInCart } from "../../Services/cartservices";
...
const FarmersCart = () => {
  ...
  const handleIncrease = (cartItemId: number, quantity: number, productId: number) => {
  ...
  const handleDecrease = async (cartItemId: number, quantity: number, productId: number) => {
    if (quantity > 1) {
      updateProductQuantityInCart(cartItemId, +quantity - 1, productId).then((res) => {
        getCartProducts()
          .then((res) => {
            setCart(res?.data?.values);
            calculateTotalPrice(res?.data?.values);
  ...
  const handleUpdateCartItem = async (cartItemId: number, quantity: number, productId: number) => {
    try {
      updateProductQuantityInCart(cartItemId, quantity, productId).then((res) => {
        getCartProducts()
          .then((res) => {
            setCart(res?.data?.values);
            calculateTotalPrice(res?.data?.values);
  ...
  return (
    <div className="bg-[#F5F5F5] w-full overflow-x-hidden ">
          ...
          <Grid item xs={12} md={8} order={{ xs: 2, md: 1 }}>
            <div className="bg-white  smm:shadow-lg shadow-sm rounded-md flex gap-2 flex-col">
              ...
              <div className="flex flex-col gap-y-4 bg-[#F5F5F5]">
                ...
                {cart &&
                  cart?.map((cartItem) => (
                    <div key={cartItem?.id} className="p-4 border-b flex justify-between bg-white">
                      <div className="flex flex-row smm:gap-4 gap-2">
                        ...
                        smm:w-[120px] smm:h-[120px] w-[66px] h-[66px] aspect-square object-contain"
                          ...
                          <div
                            ...
                            onClick={() => reomveProductFromCart(cartItem?.id)}
                          >
                            <DeleteForeverIcon />
                            Remove
                          </div>
                        ...
                        <div className="flex flex-col smm:gap-1 font-light gap-0">
                          <p className="font-medium text-sm smm:text-base">{cartItem?.product?.[0]?.product_name}</p>
                          <p className="text-[#515151] smm:text-base text-xs">
                            <strong>vendor: </strong>
                            {" " + cartItem?.product?.[0]?.store_info?.[0]?.store_name}
                          </p>
                      ...
                      <div className="flex flex-col justify-between  items-end">
                        <div className="text-right ">
                          ...
                          {cartItem?.product?.[0].discount_option ? (
                            <p className="font-light italic text-sm">
                              <span className="text-[#E51B48] ">{cartItem?.product?.[0]?.discount_option?.percentage}%</span> off{" "}
                              {cartItem?.product?.[0]?.discount_option?.quantity} units
                            </p>
                          ) : (
                            ""
                          )}
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            onClick={() => handleDecrease(cartItem?.id, cartItem?.quantity, cartItem?.product_id)}
                          >
                            -
                          </button>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setCart((prevCartItems: any) => prevCartItems.map((data: any) => (data.id === cartItem?.id ? cartItem : data)));
                              const element: any = document.getElementById(cartItem?.id);

                              element.disabled = false;
                              element.onclick = () =>
                                handleUpdateCartItem(cartItem?.id, cartItem?.quantity, cartItem?.product_id).then(() => (element.disabled = true));
                            }}
                          ...
                          <input
                            ...
                            className="w-[50px] smm:hidden flex h-[30px] text-center mx-auto border border-[#CECECE]"
                          />
                          <button
                            className="p-2 w-[30px] h-[30px] items-center flex justify-center rounded-md text-white bg-[#E51B48]"
                            onClick={() => handleIncrease(cartItem?.id, cartItem?.quantity, cartItem?.product_id)}
                          >
                            +
                          </button>
                        </div>
                        <div className="update hidden smm:flex">
                          <button
                            id={cartItem?.id}
            ...
            <div className="bg-white smm:shadow-lg shadow-sm rounded-md flex gap flex-col">
              ...
              <div className="border-b smm:py-4 py-2 px-4  flex flex-col gap-4">
                <div className="flex justify-between">
                  <p>Subtotal</p>
                  <p className="smm:text-base text-lg font-bold smm:font-normal text-[#70ADFF]">NGN {totalPrice}</p>
                </div>
                <div>
                  <p className="text-[#515151]">This does not include nay delivery fees yet</p>
                </div>
              </div>
              <div className="bg-[#F4F4F4] rounded-md p-4 gap-2 m-4 flex flex-col">
                ...
                <div className="flex justify-between items-center flex-wrap gap-2">
                  ...
                  <div>
                    <button className="border  border-black py-2 px-4 rounded-md">NGN {totalPrice}</button>
                  </div>
                </div>
              </div>
              <button
      ...
      <div className="smm:mx-4  sm:w-full bg-white">
        <Category styles={"text-black bg-white font-medium text-lg"} title={"Related to this item"} country={"country"} />
      </div>
      <div className="smm:mx-4  sm:w-full bg-white">
        <Category styles={"text-black bg-white font-medium text-lg"} title={"Recently Viewed"} country={"country"} />
      </div>
