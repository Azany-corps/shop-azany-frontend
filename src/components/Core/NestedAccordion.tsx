...
const Accordion: React.FC<AccordionProps> = ({ title, children, subcategories, handleCategorySelect }) => {
    ...
    return (
        <div className="mb-2">
            <div
                ...
                onClick={() => { subcategories.length === 0 ? handleCategorySelect(title) : toggleAccordion() }}
            >
                <span>{title}</span>
    ...
    return (
        <>
            {categories.map(category => (
                <Accordion key={category.id} title={`${'--'.repeat(level)} ${category.title}`} subcategories={category.subcategories} handleCategorySelect={handleCategorySelect}>
