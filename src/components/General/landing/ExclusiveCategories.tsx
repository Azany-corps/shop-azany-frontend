...
interface ExclusiveCategoriesProps {
  styles: string;
  title: string;
  country: string;
}

const ExclusiveCategories = ({ styles, title, country }: ExclusiveCategoriesProps) => {
  ...
  useEffect(() => {
    if (country) {
      const fetchProducts = async () => {
        try {
          setIsLoading(true);
          const response = await callAPI(`general/products/list_exclusive_deal_product/${country}`, "GET", null);
